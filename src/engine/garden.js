...
// Hand-drawn grass and water (the tileset has no outdoor tiles).
// Colours are sampled from the pack so it matches.
...
// Stone kerb on whichever sides face out of the bed.
...
// Shallows where water meets the bank.
...
export function drawGardens(ctx) {
  ...
  // Edges last, on top of the fills.
  for (const pond of PONDS) {
    for (let ty = pond.y; ty < pond.y + pond.h; ty++) {
