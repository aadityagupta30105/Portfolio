...
// Canvas renderer and game loop. Reports zone/room changes upward so React
...
export default function GameWorld({ inputRef, frozen, onNear, onEnterRoom }) {
  ...
  // Keyboard and touch pad share one input object.
  useEffect(() => {
    const set = (e, value) => {
      // leave browser shortcuts alone; an open panel owns the keys
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // integer zoom keeps pixels aligned
      ...
      // sort by feet so lower characters overlap those behind
      const cast = [
