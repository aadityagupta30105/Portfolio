...
// Traffic-light button with a padded hit area.
...
// Terminal window opened by a room console.
export default function SectionPanel({ section, onClose }) {
  ...
  // Move focus in so the keyboard scrolls the panel, and restore it on close.
  useEffect(() => {
    if (!View) return;
  ...
  return (
    <motion.div
      ...
      // flex, not grid, so max-height actually constrains the body
