// Headless movement checks: walking, collision, sliding, console prompts.
...
// 2. Holding a direction moves at ~SPEED px/s.
{
  const p = createPlayer();
  const y0 = p.y;
...
// 3. Diagonal is normalised, not faster than straight.
{
...
// 7. Prompt from every standable side.
for (const room of ROOMS) {
